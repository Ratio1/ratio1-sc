...
describe("NDContract", function () {
	...
	*/
	...
	beforeEach(async function () {
		...
		ndContract = await NDContract.deploy(naeuraContract.address, backend.address);
	...
	*/
	...
	it("Buy license - insufficent allowance", async function () { 
		await naeuraContract.connect(owner).mint(firstUser.address, ONE_TOKEN.mul(500));
		await expect(
	...
	it.skip("Buy all license ", async function () {// for gas test remove this function using "it.skip"
		//SETUP WORLD
		...
		//DO TEST
		await expect(
			buyLicenseWithMintAndAllowance(naeuraContract, ndContract, owner, firstUser,(await ndContract._priceTiers(1)).usdPrice.toNumber()*maxUnits+1,maxUnits+1,1,signedMessage)
		...
		for (let i = 1;i<=12;i++){
			...
			do{
				if (units > maxUnits){
					await buyLicenseWithMintAndAllowance(naeuraContract, ndContract, owner, firstUser,tier.usdPrice.toNumber()*maxUnits,maxUnits,i,signedMessage);
					units -=maxUnits
				} else {
					await buyLicenseWithMintAndAllowance(naeuraContract, ndContract, owner, firstUser,tier.usdPrice.toNumber()*units,units,i,signedMessage);
					units -=units
				}
		...
		await expect(
			buyLicenseWithMintAndAllowance(naeuraContract, ndContract, owner, firstUser,20000,1,12,signedMessage)
