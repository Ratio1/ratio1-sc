...
describe("MNDContract - claimRewards module", function () {
  async function deploy() {
    ...
    const mndContract = await MNDContract.deploy(
      tokeContract.address,
      ONE_DAY_IN_SECS
    );
  ...
  it("call claimRewards is 0 for the first 4 months", async function () {
    ...
    await mndContract
      ...
      .registerNode(0, SECOND_USER_NODE_HASH);
    ...
    for (let i = 0; i < 100; i++) {
      myComputeRewardsParams.nodeAvailabilities.push({
        epoch: i,
        availability: 255,
      });
    }

    const estimatedRewards = await mndContract.estimateRewards(
      secondSigner.address,
      [myComputeRewardsParams]
    );
    ...
    const estimatedRewards2 = await mndContract.estimateRewards(
      secondSigner.address,
      [myComputeRewardsParams]
    );
  ...
  it("call claimRewards for the first year should include the first 4 months", async function () {
    ...
    expect(estimatedRewards[0].masterLicenseId).to.eq(0);
    expect(estimatedRewards[0].rewardsAmount).to.eq(
      TOTAL_EMISSION_PER_MASTER_LICENSE.div(5)
    );
  });
});*/
